"""Generated SANDBOX test t-184-1 -- #184 repeatable-field preservation.

Live-SANDBOX proof of the issue #184 fix: updating one instance of a
repeatable MARC tag must NOT delete the other instances. The test seeds a
throwaway bib carrying three 650 subject headings (Physics / Astrophysics /
Cosmology), calls ``update_marc_field(mms_id, "650", {"a": ...})`` in the
default non-destructive ``replace_first`` mode, RE-FETCHES the record, and
asserts all three 650s survive with only the first occurrence rewritten.
Pre-fix, ``_build_updated_marc_xml`` replaced the first 650 and continued
past the rest, collapsing three subject headings to one silently.

R9: the seed bib is created by the test itself and carries only synthetic
MARC data -- no operator-supplied identifiers are baked in. ``test-data.json``
is loaded at runtime for consistency with other chunks (these self-seeding
tests reference no fixture keys). The verification RE-FETCHES with
``get_record`` and parses the returned MARCXML rather than trusting the
update echo (GET/PUT asymmetry).
...
def test_t_184_1():
    ...
    try:
        ...
        print(f"[t-184-1] CREATED MMS_ID: {created_mms_id}")
        ...
        # --- act: non-destructive replace_first of the 650 tag --------------
        update_response = bibs.update_marc_field(
            created_mms_id, "650", {"a": "Physics -- data reduction"}
        )
        ...
        assert len(subj_650) == 3, (
            f"expected all three 650s to survive (the #184 symptom is a 3->1 "
            f"collapse), got {subj_650!r}"
        )
        assert subj_650[0] == "Physics -- data reduction", (
            f"first 650 occurrence should be rewritten in place, got {subj_650!r}"
        )
        assert "Astrophysics" in subj_650, f"lost untargeted heading: {subj_650!r}"
        assert "Cosmology" in subj_650, f"lost untargeted heading: {subj_650!r}"
    finally:
        _delete_seed(bibs, created_mms_id, "t-184-1")

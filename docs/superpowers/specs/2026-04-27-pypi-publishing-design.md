...
### 7.3 Wheel/sdist inclusion list (D8 — Option 2: allowlist, not denylist)

Per the inclusion-vs-exclusion discussion on 2026-04-27, the build artifacts contain exactly the items below — and nothing else. The audit and Phase 1.5 inspection verify this against the actual wheel and sdist contents, hard-halting on any deviation.

**Wheel** (`almaapitk-0.3.0-py3-none-any.whl`)

- `src/almaapitk/**/*.py` — the entire package source
- Standard wheel metadata (`*.dist-info/METADATA`, `*.dist-info/RECORD`, etc.) — generated by Poetry, not under our direct control

This is automatic from `[tool.poetry] packages = [{ include = "almaapitk", from = "src" }]`. No additional configuration is required for the wheel to be a clean allowlist.

**Pre-publish cleanup already done (commit `30b919f`, 2026-04-27):** Three documentation files (`README.md`, `LOGGING_IMPLEMENTATION_PLAN.md`, `LOG_VIEWER_GUIDE.md`) that lived inside `src/almaapitk/alma_logging/` were moved to `docs/alma_logging/` so the wheel contains zero non-Python content. As of that commit, `find src/almaapitk -type f ! -name "*.py" ! -path "*/__pycache__/*"` returns zero results.

**Sdist** (`almaapitk-0.3.0.tar.gz`)

- `src/almaapitk/**` — package source (same as wheel)
- `pyproject.toml` — build instructions; auto-included by every PEP 517 backend
- `README.md` — long description for the PyPI page
- `LICENSE` — legal requirement
- `docs/releases/0.3.0.md` — release notes for context

The configuration to enforce this (Phase 1.3 step) looks approximately like:
...
The exact syntax may need adjustment — Poetry 2.x's interaction between `packages`, `include`, and `exclude` for sdists is a known fiddly area and is flagged in §17 as an open item. Phase 1.5 inspection is the safety net.

**Audit (Phase 0) verification:** The wheel's contents must match exactly `src/almaapitk/**/*.py` plus standard wheel metadata. The sdist's contents must match exactly the five items listed above. Any deviation is a hard halt — either the configuration is wrong, or a file landed in the source tree that shouldn't have.

**Why this is better than a denylist:** Adding a new file to the repo doesn't expand the ship list unless we also change the inclusion config. The default fails closed (file is shipped only if explicitly named), not open (file is shipped unless we remembered to exclude it).
...
| 1.3 | Configure the inclusion list (see §7.3). For Poetry, this means `[tool.poetry] include = [...]` for sdist additions and `exclude = ["**/*"]` to flip from denylist to allowlist semantics. The wheel is already an allowlist via `packages = [...]`. | Configuration disagreement with §7.3 |
...
| 1.5 | `rm -rf dist/ && poetry build`. Inspect `unzip -l dist/*.whl` and `tar -tzf dist/*.tar.gz`. Hand-verify the contents match the §7.3 inclusion list **exactly** — neither extra files (config bug) nor missing files (e.g., LICENSE not picked up). | Wheel or sdist contents differ from §7.3 |
...
| Inclusion-list config wrong (file leaks in, or required file missed) | Medium (Poetry include/exclude semantics are finicky) | Privacy / size, or broken sdist | Phase 0 audit + Phase 1.5 wheel/sdist contents inspection both verify exact match against §7.3 |
...
- Confirm the exact `pyproject.toml` syntax for the Option-2 inclusion list under Poetry 2.x. Specifically: does `exclude = ["**/*"]` combined with explicit `include = [...]` produce the intended sdist contents, or does Poetry's matching logic need a different incantation? The wheel side is straightforward (`packages = [{ include = "almaapitk", from = "src" }]` is already correct). The sdist side is the fiddly one. Phase 1.5 contents inspection is the verification gate.

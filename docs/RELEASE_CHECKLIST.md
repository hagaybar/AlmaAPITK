...
- [ ] `poetry run pytest tests/unit/ tests/logging/ tests/integration/client/ -q` — 0 failures, 0 errors (not just "skipped"). If pre-existing broken-test files surface, move them out of `tests/` (e.g., to `scripts/investigations/`) and document in CHANGELOG.
